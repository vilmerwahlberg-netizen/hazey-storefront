    ...
    /* ── Hero-datakällemigrering (2026-09-10) ──
       Heron läser nu i FÖRSTA HAND riktiga slides direkt ur Nyehandels
       egen native bildspels-DOM (se nhReadNativeHeroSlides längre ner) --
       INTE längre den hårdkodade kampanjkonfigurationen nedan som
       primär källa. NH_HERO_CAMPAIGNS_FALLBACK finns KVAR ENDAST som
       skyddsnät (se nhActiveHeroSlides/initHomepageV2): används BARA om
       native-avläsningen misslyckas eller ger noll giltiga slides, ALDRIG
       när riktiga native-slides finns. Rör inte renderings-/kub-logiken
    ...
    /* ── Läser RIKTIGA hero-slides direkt ur Nyehandels egen native
       bildspels-DOM (.template-components__slideshow .slideshow), i
       stället för NH_HERO_CAMPAIGNS_FALLBACK ovan. Ren avläsningsfunktion
       -- rör ALDRIG native-DOM:en (ingen mutation här), returnerar bara
       enkla dataobjekt. Rubrik/underrubrik/CTA-text/CTA-länk är
       Nyehandel-adminens EGNA, redan sparade, riktiga värden -- ingen
       fabricerad text.
    ...
    function nhReadNativeHeroSlides(slideshow) {
      ...
      var read = slideEls.map(function (el) {
        ...
        var ctaHref = actionA ? actionA.getAttribute("href") : null;
        return { index: isNaN(idx) ? null : idx, image: image, title: title, sub: sub, ctaText: ctaText, ctaHref: ctaHref };
      });
      // Endast giltiga slides (bild + rubrik + fungerande CTA-text+länk)
      // -- en trasig/ofullständig adminpost hoppas tyst över, precis som
      // nhValidHeroSlide redan gör för fallback-konfigurationen. Sorteras
      // explicit på data-index (uppdragets krav "rätt ordning") snarare
      // än att lita blint på DOM-ordningen.
      return read
    ...
    /* Konverterar en läst native-slide (se ovan) till samma objektform
       ...
       alt-text fabricerad -- aria-label utelämnas helt när ingen finns. */
    ...
    function nhHeroSlideHtml(slide, i, kampanjerHref, catLinks) {
      ...
      // Native hero-slides har INGEN exponerad alt-text i den renderade
      // DOM:en (verifierat 2026-09-10) -- fabricerar aldrig en egen
      // beskrivning, aria-label utelämnas då helt i stället för ett
      // gissat värde.
      var ariaLabelAttr = slide.alt ? ' aria-label="' + slide.alt.replace(/"/g, "&quot;") + '"' : "";
      ...
      // ett litet, omotiverat tomrum ovanför rubriken.
      ...
      return '<div class="nh-hero-slide ' + slideKindClass + '" role="group" aria-roledescription="slide"' + ariaLabelAttr + ' aria-hidden="' + (i === 0 ? "false" : "true") + '" data-theme="' + (slide.theme || "default") + '"' + bg + '>'
    ...
    function nhHeroHtml(navData, kampanjerHref, active) {
      ...
      // parameter (redan upplöst i initHomepageV2 -- native-slides i
      // första hand, NH_HERO_CAMPAIGNS_FALLBACK bara om avläsningen gav
      // noll giltiga resultat) i stället för att denna funktion själv
      // ringde nhActiveHeroSlides(). Renderings-/kub-logiken nedan är
      // HELT oförändrad oavsett vilken källa slidesen kom ifrån.
