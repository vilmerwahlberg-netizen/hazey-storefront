    ...
    function nhPopularaSerierHtml(navData) {
      ...
      // KORRIGERINGSRUNDA (2026-09-15, Paket 3+4): solikonen satt tidigare
      // INUTI kickern (liten, 15px, bredvid "Upptäck mer") -- facit visar
      // den i stället som en avsiktlig, större del av SJÄLVA rubrik-
      // kompositionen, direkt efter "Populära serier". Flyttad in i H2:n
      // (samma SVG-path, oförändrad) + ny egen klass (.nh-pser-sun) för
      // rubrik-skalig storlek i CSS, i stället för kickerns tidigare
      // .nh-pser-kicker__sun (borttagen).
      return '<section class="nh-pser section-gap" id="populara-serier">'
        + '  <div class="sec-head">'
        + '    <div><span class="nh-pser-kicker">Upptäck mer</span>'
        + '    <h2>Populära serier<svg class="nh-pser-sun" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" aria-hidden="true"><circle cx="12" cy="12" r="4.2"/><path d="M12 2.5v3M12 18.5v3M21.5 12h-3M5.5 12h-3M18.5 5.5l-2.1 2.1M7.6 16.4l-2.1 2.1M18.5 18.5l-2.1-2.1M7.6 7.6L5.5 5.5"/></svg></h2></div>'
    ...
    // KORRIGERINGSRUNDA (2026-09-15, Paket 3+4): kicker "Våra favoriter"
    // + en dekorativ eldikon (samma rent kosmetiska, datafria mönster som
    // solen vid Populära serier/kickern vid Spotlight/Reviews -- ingen ny
    // datapunkt, bara ett visuellt grepp) tillagda för att ge sektionen
    // samma redaktionella rubriktyngd som de redan godkända Spotlight-/
    // Reviews-sektionerna. Den befintliga, riktiga mikrotexten ("Redo att
    // skickas idag...") TAS INTE BORT (uppdragets uttryckliga förbud mot
    // att radera innehåll) -- bara nedtonad till sekundär mikrocopy via
    // en ny klass (.nh-featured-micro), se CSS. CTA-texten bytt till
    // "Se alla produkter →" (uppdragets explicita copy för denna sektion).
    function nhBestsellersHtml() {
      var flameIcon = '<svg class="nh-featured-flame" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="M12 3c1.2 2.6-2.4 4-2.4 7a2.4 2.4 0 004.8 0c0-.9-.5-1.6-.7-2.4 1.7 1 3.1 3 3.1 5.4a4.8 4.8 0 01-9.6 0c0-4.2 3-6.3 4.8-10z"/></svg>';
      return '<section class="nh-featured section-gap" id="nh-featured">'
        + '  <div class="sec-head"><div><span class="nh-featured-kicker">Våra favoriter</span>'
        + '  <h2>Bästsäljare i lager' + flameIcon + '</h2>'
        + '  <p class="nh-featured-micro">Redo att skickas idag — inget variantval krångel.</p></div>'
        + '  <a class="more" href="/sv/categories/alla-produkter">Se alla produkter →</a></div>'
    ...
    /* ── "I rampljuset" (Spotlight) -- datakällemigrering (2026-09-15) ──
       Läser nu i FÖRSTA HAND en riktig native "Text med bild"-sektion
       (Nyehandels egen page builder-komponent) i stället för att fetch:a
       en enskild produkts JSON-LD. Se nhReadNativeSpotlight nedan för
       den fullständiga avläsningen/valideringen.

       NH_SPOTLIGHT_FALLBACK/nhSpotlightHtmlFallback/nhInitSpotlightFallback
       BEHÅLLS i koden, tydligt namngivna, som ett manuellt skyddsnät --
       används BARA om native-sektionen saknas, är ofullständig eller har
       en osäker CTA-länk (se initHomepageV2). Ta inte bort förrän Vilmer
       visuellt godkänt migreringen. */
    /* ── Historik (2026-09-07/08, den ursprungliga, fortfarande aktiva
       FALLBACK-konfigurationen nedan) ──
       Uttryckligt krav då: välj INTE automatiskt en teknisk/tråkig produkt
       bara för att den råkar ligga först i bästsäljarlistan -- kräver
       en explicit konfiguration eller en verifierad flagga. Produktval
       enligt uppdragets egen prioritetsordning (relevant Magic Sauce-
       produkt först): "Vape - Magic Sauce 99% - 2ml" -- verifierat via
       produktens riktiga JSON-LD (curl 2026-09-08): I lager, 595 kr,
       betyg 4,69/5 (13 omdömen), varumärke Magic Farmers. Datakälla:
       samma robusta JSON-LD `<script type="application/ld+json">`-block
       som varje PDP redan skriver ut. Rationale/sekundärlänk kommer
       ALLTID från konfigen, aldrig gissat eller skrapat. */
    var NH_SPOTLIGHT_FALLBACK = {
      active: true,
      ...
      // tillbaka i lager.", se nhInitSpotlightFallback) OCH CTA-texten
    ...
    function nhReadNativeSpotlight() {
      ...
      var headingEl = iat.querySelector(".iat__content h2");
      var heading = headingEl ? headingEl.textContent.replace(/\s+/g, " ").trim() : null;
      var bodyEl = iat.querySelector(".iat__content p");
      var body = bodyEl ? bodyEl.textContent.replace(/\s+/g, " ").trim() : "";
      var ctaEl = iat.querySelector(".iat__content .action a");
    ...
    function nhSpotlightHtml(native) {
      if (native) {
        // Kampanjen gäller nu HELA Magic Sauce-serien (kategori-CTA från
        // native-sektionen), inte den enskilda produkten "Vape - Magic
        // Sauce 99% - 2ml" -- typ/pris/lager/betyg (#nhSpotlightType/
        // #nhSpotlightMeta) hörde till DEN produkten specifikt och skulle
        // vara en felaktig semantisk blandning ihop med den nya
        // kategori-länken (uppdragets uttryckliga krav). Elementen finns
        // kvar i DOM:en (samma ids, ingen markup tas bort) men renderas
        // `hidden` och lämnas HELT tomma -- ingen efterföljande kod
        // fyller i dem för native-grenen.
        //
        // Mobil och desktop använder SAMMA native-bild (samma princip
        // som redan gäller för heron, se STATUS.md 2026-09-10) -- ingen
        // gissad separat mobilbeskärning. Bild-URL:en saneras med samma
        // nhSanitizeCssUrl som heron (kontrolltecken/citattecken/
        // backslash, rätt ordning för ett CSS url()-token i ett HTML-
        // attribut). Rubrik/brödtext/CTA-text HTML-escapas (nhEscHtml),
        // CTA-länken attribut-escapas (nhEscAttr) -- den är dessutom
        // redan schema-validerad (nhSafeHref) i nhReadNativeSpotlight.
        //
        // Ingen sekundärlänk för native-grenen: den gamla "Se hela Magic
        // Sauce-sortimentet →" pekade mot EXAKT samma kategori-URL som
        // den nya primära CTA:n nu gör -- att visa båda hade varit en
        // meningslös dubblettlänk. Sekundärlänken hör bara hemma i
        // fallback-grenen (se nedan), som fortfarande har en riktig,
        ...
        return '<section class="nh-spotlight section-gap" id="nh-spotlight" data-nh-spotlight-source="native">'
          ...
          + '      <p class="nh-spotlight-type" id="nhSpotlightType" hidden></p>'
          + '      <h2 id="nhSpotlightName">' + headingSafe + '</h2>'
          + '      <h2 id="nhSpotlightHeadline" class="nh-spotlight-headline">' + headingSafe + '</h2>'
          + '      <p class="nh-spotlight-rationale">' + nhEscHtml(native.body) + '</p>'
          + '      <div class="nh-spotlight-meta" id="nhSpotlightMeta" hidden></div>'
          + '      <div class="nh-spotlight-cta">'
          + '        <a class="btn-solid" id="nhSpotlightBuy" href="' + nhEscAttr(native.ctaHref) + '">'
          + '          <span id="nhSpotlightBuyLabel">' + nhEscHtml(native.ctaText) + '</span>'
      ...
      // ── FALLBACK (oförändrad sedan 2026-09-10, se NH_SPOTLIGHT_FALLBACK
      // ovan) -- används bara när nhReadNativeSpotlight() gav null. ──
      if (!NH_SPOTLIGHT_FALLBACK.active || !NH_SPOTLIGHT_FALLBACK.productHref) return "";
      return '<section class="nh-spotlight section-gap" id="nh-spotlight" data-nh-spotlight-source="fallback">'
        + '  <div class="nh-spotlight-inner">'
        + '    <div class="nh-spotlight-media">'
        ...
        + '      <p class="nh-spotlight-rationale">' + NH_SPOTLIGHT_FALLBACK.rationale + '</p>'
        + '      <div class="nh-spotlight-meta" id="nhSpotlightMeta"></div>'
        + '      <div class="nh-spotlight-cta">'
        + '        <a class="btn-solid" id="nhSpotlightBuy" href="' + NH_SPOTLIGHT_FALLBACK.productHref + '">'
        + '          <span id="nhSpotlightBuyLabel">Köp ' + NH_SPOTLIGHT_FALLBACK.seriesLabel + '</span>'
        + '          <span aria-hidden="true">→</span>'
        + '        </a>'
        + (NH_SPOTLIGHT_FALLBACK.secondaryHref
            ? '<a class="hero-link nh-spotlight-secondary" href="' + NH_SPOTLIGHT_FALLBACK.secondaryHref + '">' + NH_SPOTLIGHT_FALLBACK.secondaryLabel + '</a>'
            : '')
        + '      </div>'
        + '    </div>'
        + '  </div>'
    ...
    /* FALLBACK-ENDAST (se filens header-kommentar ovan) -- den gamla,
       per-produkt-fetch-baserade berikningen. Anropas INTE när native-
       sektionen gav giltig data (se initHomepageV2) -- oförändrad. */
    function nhInitSpotlightFallback(root) {
      if (!NH_SPOTLIGHT_FALLBACK.active || !NH_SPOTLIGHT_FALLBACK.productHref) return;
      var section = root.querySelector("#nh-spotlight");
      if (!section) return;
      fetch(NH_SPOTLIGHT_FALLBACK.productHref, { credentials: "same-origin" })
        .then(function (r) { return r.ok ? r.text() : ""; })
        .then(function (html) {
          if (!html) throw new Error("no html");
          var doc = new DOMParser().parseFromString(html, "text/html");
          ...
          section.querySelector("#nhSpotlightHeadline").textContent = NH_SPOTLIGHT_FALLBACK.seriesLabel
    ...
    function nhInitProductReviews(root) {
      ...
      function renderReal(real) {
        ...
        function renderPage() {
          ...
          //
          // ESCAPING-FIX (2026-09-15): återanvänder samma redan etablerade
          // säkringshjälpare som resten av filen (se toppen: nhEscHtml/
          // nhEscAttr/nhSafeHref, byggda för hero-slidedata) i stället för
          // ett eget, ofullständigt `<`-only-filter -- namn/citat/
          // produktnamn är RIKTIG, extern kunddata (samma tillitsnivå som
          // hero-slidedata) och ska HTML-escapas fullt ut (&/</>), inte
          // bara delvis. `title`-attributet är dessutom ett citerat HTML-
          // attribut (kräver även "-escaping, nhEscHtml räcker inte där).
          // Länken saneras med nhSafeHref (blockerar javascript:/data:
          // m.fl.) -- osannolikt att den någonsin är ogiltig här (kommer
          // från en riktig <a href> på en verklig produktsidelista), men
          // faller ärligt tillbaka på en olänkad <span> med produktnamnet
          // kvar i stället för att antingen rendera en farlig länk eller
          // tyst tappa bort en annars äkta recension.
          grid.innerHTML = pages[pageIndex].map(function (r) {
            ...
            var safeHref = nhSafeHref(r.href);
            var productHtml = safeHref
              ? '<a href="' + nhEscAttr(safeHref) + '" title="' + nhEscAttr(r.product) + '">' + nhEscHtml(r.product) + '</a>'
              : '<span>' + nhEscHtml(r.product) + '</span>';
            return '<div class="nh-rc-card">'
              + starsHtml
              + '<p class="nh-rc-quote">”' + nhEscHtml(r.text) + '”</p>'
              + '<div class="nh-rc-source"><span class="nh-rc-name">' + nhEscHtml(r.name) + '</span>'
              + productHtml + '</div>'
              + '</div>';
          }).join("");
          grid.hidden = false;
    ...
    /* KORRIGERINGSRUNDA (2026-09-15): rotorsak till att hela sektioner
       kunde förbli osynliga (stora tomma creamytor, rapporterat bl.a.
       mellan Snabb koll och FAQ) vid snabb scroll/Page Down/stora
       scrollhopp -- den tidigare koden hade BARA EN väg ut: ett enda
       `scroll`-event, rAF-strypt till max en sweep per bildruta, utan
       någon "vila"-bekräftelse. Verifierade Chromium-beteenden gör detta
       skört: webbläsaren kan koalescera/strypa `scroll`-events under en
       snabb fling-scroll (särskilt med en passiv lyssnare) så att det
       SISTA, avgörande eventet för en viss gest aldrig levereras -- om
       det händer fastnar elementet permanent (inget mer scroll-event
       kommer någonsin trigga en ny sweep). Samma sak gäller en ankarlänk
       som redan låg i URL:en vid sidladdning (webbläsarens egen hopp kan
       ha skett innan vår scroll-lyssnare ens hann bindas).
       Fix: sweepen triggas nu från FLERA oberoende källor i stället för
       bara `scroll`, så att MINST en av dem alltid når fram:
       1) `scroll` + rAF (oförändrad, snabb respons under pågående scroll).
       2) en ren `setTimeout`-baserad "vila"-koll (oberoende av om ännu
          ett scroll-/paint-event någonsin kommer) -- den riktiga fixen
          för koalescerings-scenariot ovan.
       3) `scrollend` där webbläsaren stödjer den (byggd exakt för detta
          "har scrollningen verkligen avslutats"-problem).
       4) `resize`, `hashchange` och `pageshow` (bfcache-återkomst) --
          täcker ankarlänkar och en sida som visas igen från bakåt/framåt-
          cachen utan att köra om boot-skriptet.
       Ingen av dessa ändrar SJÄLVA animationen (.pre-reveal/.in-view-CSS:en
       är orörd) -- bara HUR OFTA/PÅ VILKA SÄTT vi kontrollerar om något
       redan borde vara synligt. */
    var revealSweepQueued = false;
    var revealSettleTimer = null;
    function queueRevealSweep() {
      ...
      revealSettleTimer = setTimeout(revealPassedElements, 180);
    }
    window.addEventListener("scroll", queueRevealSweep, { passive: true });
    window.addEventListener("resize", queueRevealSweep, { passive: true });
    window.addEventListener("hashchange", queueRevealSweep);
    window.addEventListener("pageshow", queueRevealSweep);
    if ("onscrollend" in window) {
      window.addEventListener("scrollend", revealPassedElements, { passive: true });
    }
    ...
    function initHomepageV2() {
      ...
      // FALLBACK-ENDAST: körs bara när native-sektionen INTE gav giltig
      // data -- den redan renderade native-grenen behöver ingen
      // efterhandsberikning (allt lästes synkront).
      if (!nativeSpotlight) nhInitSpotlightFallback(document);
